...
LOCAL void uart1_rx_intr_handler(void *para);
...
{
    if (uart_no == UART1) {
    	//ETS_UART_INTR_ATTACH(uart1_rx_intr_handler,  &(UartDev.rcv_buff));
...
uart0_rx_intr_handler(void *para)
{
    ...
    if (UART_RXFIFO_FULL_INT_ST == (READ_PERI_REG(UART_INT_ST(uart_no)) & UART_RXFIFO_FULL_INT_ST)) {
        ...
        //Artem tx_start_uart_buffer(uart_no);
...
LOCAL void
uart1_rx_intr_handler(void *para)
{
	uint8 uart_no = UART1;
	WRITE_PERI_REG(UART_INT_CLR(uart_no), 0xffff);
